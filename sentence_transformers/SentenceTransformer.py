...
class SentenceTransformer(nn.Sequential):
    ...
    def encode(self, sentences: Union[str, List[str], List[int]],
        ...
        for features in iterator:
            ...
            with torch.no_grad():
                ...
                if output_value == 'token_embeddings':
                    ...
                    logger.info(input_mask_expanded.shape)

                embeddings = embeddings.detach()
                if output_value == 'intra_similarity':
                    input_mask = out_features['attention_mask']
                    embeddings_tokens = out_features['token_embeddings']
                    input_mask_expanded = input_mask.unsqueeze(-1).expand(embeddings_tokens.size()).float()
                    embeddings_tokens = embeddings_tokens * input_mask_expanded
                    cos = torch.matmul(embeddings_tokens, embeddings.unsqueeze(2)).squeeze(2) /\
                           (torch.norm(embeddings_tokens, dim=2) * torch.norm(embeddings, dim=1, keepdim=True) + 1e-8)
                    # logger.info(cos)
                    intra_similarity = torch.sum(cos, dim=1, keepdim=True) / (torch.sum(input_mask, dim=1, keepdim=True) + 1e-8)  # (Batch, )
                    intra_similarity = intra_similarity.cpu()
                    intra_similarities.extend(intra_similarity)
                    # logger.info(intra_similarity)
        ...
        if output_value == 'intra_similarity':
            intra_similarities = [intra_similarities[idx] for idx in np.argsort(length_sorted_idx)]
            if convert_to_tensor:
                intra_similarities = torch.stack(intra_similarities)
            elif convert_to_numpy:
                intra_similarities = np.asarray([emb.numpy() for emb in intra_similarities])

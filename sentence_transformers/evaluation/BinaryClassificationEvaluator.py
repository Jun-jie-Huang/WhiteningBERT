...
class BinaryClassificationEvaluator(SentenceEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        for name, scores, reverse in [['Cosine-Similarity', cosine_scores, True]]:
            ...
            acc_thre = self.acc_given_threshold(scores, labels, reverse, threshold=0.5)
            logging.info("Accuracy with {}:           {:.2f}\t(Threshold: {:.4f})".format(name, acc * 100, acc_threshold))
            logging.info("Accuracy with {}:           {:.2f}\t(Threshold: 0.5000)".format(name, acc_thre * 100))
            acc_thre = self.acc_given_threshold(scores, labels, reverse, threshold=acc_threshold)
            logging.info("Accuracy with {}:           {:.2f}\t(Threshold: {:.4f})".format(name, acc_thre * 100, acc_threshold))
            ...
            file_output_data.extend([acc_thre, acc, acc_threshold, f1, precision, recall, f1_threshold, ap])
    ...
    @staticmethod
    def acc_given_threshold(scores, labels, high_score_more_similar: bool, threshold: float = 0.5):
        assert len(scores) == len(labels)
        rows = list(zip(scores, labels))
        rows = sorted(rows, key=lambda x: x[0], reverse=high_score_more_similar)

        preds = [float(i > threshold) for i in scores]
        acc = sum([i == j for i, j in zip(labels, preds)]) / len(labels)
        # logger.info(preds)
        # logger.info(scores)
        # logger.info(labels)
        return acc
...
class WhiteningBinaryClassificationEvaluator(SentenceEvaluator):
    """
    Evaluate a model based on the similarity of the embeddings by calculating the accuracy of identifying similar and
    dissimilar sentences.
    The metrics are the cosine similarity as well as euclidean and Manhattan distance
    ...
    def __init__(self, sentences1: List[str], sentences2: List[str], labels: List[int], name: str = '', batch_size: int = 32, show_progress_bar: bool = False, embed_dim: int = 768):
        self.sentences1 = sentences1
        self.sentences2 = sentences2
        self.labels = labels

...
class WKPooling(nn.Module):
    ...
    def forward(self, features: Dict[str, Tensor]):
        ...
        for sent_index in range(len(unmask_num)):  # len(unmask_num)=16
            ...
            for token_index in range(sentence_feature.shape[1]):
                token_feature = sentence_feature[:, token_index, :]  # (9, 768)
        ...
        output_vector = torch.stack(embedding).to(org_device)  # (Batch, 768)
